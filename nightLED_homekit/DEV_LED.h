...
struct DEV_DimmableLED : Service::LightBulb {       // Dimmable LED
  ...
  DEV_DimmableLED(int pin) : Service::LightBulb(){       // constructor() method
    ...
    level=new Characteristic::Brightness(100);       // NEW! Instantiate the Brightness Characteristic with an initial value of 50% (same as we did in Example 4)
    level->setRange(0,100,10);                       // NEW! This sets the range of the Brightness to be from a min of 5%, to a max of 100%, in steps of 1% (different from Example 4 values)
