...
//GPIO for boot button, this is used to reset rainmaker
#if CONFIG_IDF_TARGET_ESP32C3
static int gpio_boot = 9;
#else
static int gpio_boot = 0;
#endif

// lightbulb pin
static int gpio_dimmer = 10;
bool dimmer_state = false;

int dimmer_level_arduino = 255;
int dimmer_level_percent = 100;
...
void write_callback(Device *device, Param *param, const param_val_t val, void *priv_data, write_ctx_t *ctx)
{
    ...
    if (strcmp(param_name, "Power") == 0) {
        ...
        dimmer_level_percent = val.val.i;
        dimmer_level_arduino = map(dimmer_level_percent, 0, 100, 0, 255);
        param->updateAndReport(val);
    }
    analogWrite(gpio_dimmer, dimmer_level_arduino*dimmer_state);
}

void setup()
{
    Serial.begin(115200);
    pinMode(gpio_boot, INPUT);
    pinMode(gpio_dimmer, OUTPUT);
    digitalWrite(gpio_dimmer, DEFAULT_POWER_MODE);

    Node my_node;
    my_node = RMaker.initNode("ESP RainMaker Node");
    my_device = new Device("NightLED", "esp.device.lightbulb", &gpio_dimmer);
    if (!my_device) {
        return;
    }
...
void loop()
{
    if (digitalRead(gpio_boot) == LOW) { //Push button pressed

        // Key debounce handling
        delay(100);
        int startTime = millis();
        while (digitalRead(gpio_boot) == LOW) {
            delay(50);
        }
        int endTime = millis();
